...
- (void)layoutSubviews
{
    ...
    switch (self.model.bodyType) {
        ...
        {
            [self removeConstraint:self.bubbleWithImageConstraint];
            if ([EaseMessageCell appearance].voiceCellWidth + self.model.mediaDuration *10>200) {
                 self.bubbleWithImageConstraint = [NSLayoutConstraint constraintWithItem:self.bubbleView attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:200];
            }else{
                 self.bubbleWithImageConstraint = [NSLayoutConstraint constraintWithItem:self.bubbleView attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:[EaseMessageCell appearance].voiceCellWidth + self.model.mediaDuration *10];
            }
