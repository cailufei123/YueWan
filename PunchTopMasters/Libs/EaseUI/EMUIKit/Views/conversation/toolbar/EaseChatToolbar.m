...
- (void)_setupSubviews
{
    ...
    _backgroundImageView.image = [[UIImage imageNamed:@"EaseUIResource.bundle/messageToolbarBg"] stretchableImageWithLeftCapWidth:0.0 topCapHeight:10];
    ...
    _inputTextView.layer.cornerRadius = 0.0f;
    ...
    phoneButton.accessibilityIdentifier = @"phoneButton";
    phoneButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
    [phoneButton setImage:[UIImage imageNamed:@"微信图片_20180518155300"] forState:UIControlStateNormal];
  
    [phoneButton addTarget:self action:@selector(phoneButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    
    EaseChatToolbarItem *phoneButtonItem = [[EaseChatToolbarItem alloc] initWithButton:phoneButton withView:nil];

    [self setInputViewLeftItems:@[phoneButtonItem]];
...
//    but.accessibilityIdentifier = @"more";
//    but.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
//    [but setImage:[UIImage imageNamed:@"EaseUIResource.bundle/chatBar_more"] forState:UIControlStateNormal];
//    [but setImage:[UIImage imageNamed:@"EaseUIResource.bundle/chatBar_moreSelected"] forState:UIControlStateHighlighted];
//    [but setImage:[UIImage imageNamed:@"EaseUIResource.bundle/chatBar_keyboard"] forState:UIControlStateSelected];
//    [but addTarget:self action:@selector(moreButtonAction:) forControlEvents:UIControlEventTouchUpInside];
//    EaseChatToolbarItem * moreItem2 = [[EaseChatToolbarItem alloc] initWithButton:but withView:self.moreView];
...
- (void)phoneButtonAction:(id)sender{//点击图片按钮
//     UIButton *button = (UIButton *)sender;
    if (_delegate && [_delegate respondsToSelector:@selector(moreViewPhotoAction:)]) {
        [_delegate moreViewPhotoAction:nil];
    }
