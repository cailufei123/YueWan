//
//  YWXSPlayRecordDetailsViewController.m
//  PunchTopMasters
//
...
@interface YWXSPlayRecordDetailsViewController ()
...
@property (weak, nonatomic) IBOutlet UILabel *fangjiangfeiyongLb;
@property (weak, nonatomic) IBOutlet UIButton *agenbt;
@end

@implementation YWXSPlayRecordDetailsViewController
...
-(void)viewWillAppear:(BOOL)animated{
    [super viewDidAppear:animated];
    [LFNSNOTI addObserver:self selector:@selector(loadpage) name:masterReportPlayGame object:nil];
}
-(void)loadpage{
...
-(void)arrowBtClick:(UIButton * )bt{
    if ( bt.selected) {
        ...
        self.bigBgViewLyout.constant = 30 +  self.otherArr.count * 60;
...
-(void)updataUI{
    ...
    for (int i = 0; i < self.otherArr.count; i++) {
        ...
        YWXSPlayUsersView * playUserView  = [YWXSPlayUsersView loadNameXSPlayUsersView];
    ...
    if ( self.playRecordDateilsModel.order ) {
        ...
        [self showView];
        if ([self.playRecordDateilsModel.userGameFLow.roomType isEqualToString:@"1"] ) {//AA
            ...
            if ([self.playRecordDateilsModel.userGameFLow.isMvp isEqualToString:@"0"]) {//是不是mvp
                
            }else{
                if([self.playRecordDateilsModel .userGameFLow.status isEqualToString:@"101"]){
                    self.gameDsLb.text = [NSString stringWithFormat:@"%@元的赏金赏金已近打入你的账户",self.playRecordDateilsModel .userGameFLow.winMoney];
                }else if([self.playRecordDateilsModel .userGameFLow.status isEqualToString:@"103"]){
                    self.gameDsLb.text = [NSString stringWithFormat:@"%@元的赏金赏金已近打入你的账户",self.playRecordDateilsModel .userGameFLow.winMoney];
        ...
        }else if([self.playRecordDateilsModel.userGameFLow.roomType isEqualToString:@"3"]){//悬赏
            ...
            if([self.meModel.userId isEqualToString:loginUid]){
                ...
                self.fangjiangfeiyongLb.text = @"赏金";
                self.couponTypeLb.hidden = YES;
                [self hideView];
